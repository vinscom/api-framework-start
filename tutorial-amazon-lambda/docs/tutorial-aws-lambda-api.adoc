...
----
*api-framework-amazon-lambda* is the required dependency. As we have already written
...
image::amazon-lambda/aws-lambda-config.png[]
  ...
  - *Environment Variable*
    * *GLUE_LAYERS*  :  _./lib/api-framework-1.0-SNAPSHOT-common-config.zip,./lib/tutorial-simple-get-api-1.0-SNAPSHOT-common-config.zip_
. Basic Settings
+
...
Stage will give us unique URL through which we can access our API (Later on, you can
map this stage to your custom domain) .
